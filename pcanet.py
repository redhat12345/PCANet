...
class PCANet:
    def __init__(self, image_batch, label_batch, info):
        ...
        with tf.name_scope("convolution1"):
            ...
            self.conv1_t = tf.transpose(self.conv1, [0, 3, 1, 2])
            self.conv1_batch = tf.reshape(self.conv1_t, [-1, info.IMAGE_W, info.IMAGE_H, 1])
        ...
        with tf.name_scope("convolution2"):
            ...
            self.conv2_t = tf.reshape(tf.transpose(self.conv2, [0, 3, 1, 2]), [-1, l1, l2, info.IMAGE_W, info.IMAGE_H])
            self.conv2_batch = tf.reshape(self.conv2_t, [-1, info.IMAGE_W, info.IMAGE_H, 1])
            tf.summary.image('conv2', self.conv2_batch, max_outputs=l2)

        with tf.name_scope("h1"):
            self.h1_dim = 128
            self.w_fc1 = tf.Variable(tf.truncated_normal([info.IMAGE_W * info.IMAGE_H * l1 *l2,  self.h1_dim], 0, 0.1), name='w_fc1')
            self.b_fc1 = tf.Variable(tf.constant(0.1, shape=[self.h1_dim]), name='b_fc1')
            self.conv_flat = tf.reshape(self.conv2, [-1, info.IMAGE_W * info.IMAGE_H * l1 * l2])
            self.h1 = tf.nn.relu(self.conv_flat @ self.w_fc1 + self.b_fc1, name='h1')
