...
class PCANet:
    def __init__(self, image_batch, label_batch, info):
        ...
        with tf.name_scope("extract_patches1"):
            ...
            self.zero_mean_patches1 = self.patches1 - tf.reduce_mean(self.patches1, axis=2, keep_dims=True, name='patch_means')
            x1 = tf.transpose(self.zero_mean_patches1, [1, 2, 0])
            x1_trans = tf.transpose(self.zero_mean_patches1, [1, 0, 2])
        ...
        with tf.name_scope("visualizations1"):
            ...
            tf.summary.image('conv1', self.conv1_viz, max_outputs=l1)
            tf.summary.image('filt1', self.filt1_viz, max_outputs=l1)

        # with tf.name_scope("extract_patches2"):
            # self.patches2 = tf.extract_image_patches(self.conv1_viz, [1, k1, k2, 1], strides=[1, 1, 1, 1], rates=[1, 1, 1, 1], padding='SAME', name='patches')
